...
namespace advt.Data.SqlServer
{
    public partial class DataProvider : advt.Data.IDataProvider
    {
        ...
        private string ExamRule_item_str = "[ID],[TypeName],[TotalScore],[TotalTime],[TotalSubject],[PassScore],[IsRead],[IsRepeat],[StartDeac],[EndDesc],[CreateUser],[CreateDate],[RuleName],[SubjectName]";
        ...
        {
            StringBuilder commandText = new StringBuilder();
            commandText.AppendLine(" select  [TopicMajor],[TopicLevel],[TopicType] from[dbo].[ExamBank] where ExamSubject =@model group by [TopicMajor],[TopicLevel],[TopicType]");
            List<DbParameter> l_parms = new List<DbParameter>();

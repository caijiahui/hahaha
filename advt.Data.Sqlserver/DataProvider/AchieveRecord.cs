...
namespace advt.Data.SqlServer
{
    public partial class DataProvider : advt.Data.IDataProvider
    {
        ...
        {
            StringBuilder commandText = new StringBuilder();           
            commandText.AppendLine("select * from ExamUserInfo  b inner join (select * from AchieveRecord where CreateDate >= '"+startdate+ "' and CreateDate < '" + endate + "') a on a.UserCode = b.UserCode and a.Achievement = N'符合' and b.TypeName in(N'Chassis技能等级考试',N'工控模组技能等级考试')");
